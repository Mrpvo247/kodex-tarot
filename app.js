/* KODEX TAROT – Main Controller (dark theme + full meanings + RK + cosmic) */
(() => {
  const state = {
  ...
  function renderCards(cards, target = '#cards') {
    const html = cards.map(c => `
      <div class="card ${c.reversed ? 'reversed' : ''}" data-id="${c.id}">
        <div class="card-face text-only">
          <span class="card-title">${c.name}</span>
          <span class="card-status">${c.reversed ? 'Reversed' : 'Upright'}</span>
        </div>
      </div>`).join('');
  ...
  function exportHistory() {
    ...
    const a = Object.assign(document.createElement('a'), { href: URL.createObjectURL(blob), download: 'kodex-history.json' });
    a.click();
  ...
  function init() {
    $('#dailyBtn').addEventListener('click', async () => {
      if (state.todayDraw && !state.premium) {
        alert('Free daily reading already taken. Upgrade for unlimited draws.');
        return;
      }
